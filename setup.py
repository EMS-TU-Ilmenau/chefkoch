...
def getCurrentVersion():
    ...
    if os.path.isfile(".version"):
        with open(".version", "r") as f:
            stdout = f.read().split("\n")[0]
        print(
            "Override of version string to '%s' (from .version file )"
            % (stdout)
        )
        ...
        if not os.path.exists(".git"):
            print(
                (
                    "Installing from something other than a Git repository; "
                    + "Version file '%s' untouched."
                )
                % (strVersionFile)
            )
            return
        ...
        try:
            ...
            print(
                "Not a git repository; Version file '%s' not touched."
                % (strVersionFile)
            )
            return
        ...
        if p.returncode != 0:
            print(
                (
                    "Unable to fetch version from git repository; "
                    + "leaving version file '%s' untouched."
                )
                % (strVersionFile)
            )
            return
    ...
    if versionMatch:
        ...
        print(
            "Fetched package version number from git tag (%s)."
            % (packageVersion)
        )
...
if __name__ == "__main__":
    ...
    print("Building %s v%s" % (packageName, packageVersion))
    ...
    setup(
        ...
        description=(
            "A compute cluster cuisine for distributed scientific "
            + "computing in python"
        ),
        ...
        keywords=(
            "compute cluster HPC LSF dask parallel computing "
            + "scheduler framework"
        ),
